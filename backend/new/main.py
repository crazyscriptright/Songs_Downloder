...
def download(music_url, service, quality, output, template, fallback):
    ...
    Examples:
        # Spotify URL (default - tries Tidal first)
    ...
    try:
        print("=" * 60)
        print("SpotiFLAC Python - Multi-Platform Music Downloader")
        print("=" * 60)
        ...
        # For Spotify URLs, continue with normal flow
        if detected_platform == 'spotify':
            if not track_id:
        ...
        # Step 2: Get service URL or ISRC
        ...
        if fallback and service != 'spotdl':
            ...
            # Add other services not already selected
...
def download_direct(url, platform, track_id, output_dir, template, quality):
    ...
    try:
        # Try to fetch Spotify metadata for better quality
        print("\n[1/3] Attempting to fetch Spotify metadata...")
        track_metadata = None
        try:
            songlink_client = songlink.SongLinkClient()
            # Try reverse lookup to get Spotify URL
            # This is a simplified approach - might not always work
        ...
        if platform == 'tidal':
            ...
            downloader = qobuz.QobuzDownloader()
            # Assuming track_id is the Qobuz track ID
