...
def download_video():
    ...
    try:
        ...
        return success({"url": url, "output_path": output_path, "proxy": proxy or None, "proxy_required": proxy_required, "result": result}, "Download complete", status=200 if result['success'] else 400)
...
def test_hardcoded():
    ...
    try:
        ...
        return success({"test_url": TEST_YOUTUBE_URL, "result": info}, "Test complete", status=200 if info['success'] else 400)
...
def check_status():
    ...
    """
    if not HAS_YTDLP:
        return error('yt-dlp Python library not found - install with: uv pip install yt-dlp', 500, {"installed": False})
