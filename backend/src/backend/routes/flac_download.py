...
def flac_download():
    ...
    if not url.startswith(("http://", "https://", "spotify:")):
        return error("Invalid URL — must be http/https or a spotify: URI", 400)
    ...
    if service not in valid_services:
        return error(f"Invalid service. Choose from: {', '.join(sorted(valid_services))}", 400)
    ...
    return success({"download_id": download_id, "status": "started"}, "FLAC download queued")
...
def flac_status(download_id: str):
    ...
    if not entry:
        return error("FLAC download not found", 404)
    return success(entry, f"FLAC status: {entry.get('status', 'unknown')}")
...
def flac_serve_file(download_id: str, filename: str):
    ...
    if entry.get("status") != "complete":
        return error(f"Download not complete yet (status: {entry.get('status')})", 425)
