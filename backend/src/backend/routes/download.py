...
def download_status_check(download_id):
    ...
    return success(status, f"Download {status['status']}")
...
def get_all_downloads():
    ...
    return success(filtered, f"Found {len(filtered)} downloads")
...
def bulk_download():
    ...
    return success({"bulk_id": bulk_id, "status": "started", "total": len(valid_urls)}, f"Bulk download queued ({len(valid_urls)} items)")
...
def bulk_status_check(bulk_id):
    ...
    return success(bulk_data, f"Bulk status: {bulk_data.get('status', 'unknown')}")
...
def bulk_heartbeat(bulk_id):
    if bulk_id in state.bulk_heartbeats:
        ...
        return success(None, "Heartbeat received")
    elif bulk_id in state.download_status:
        bstatus = state.download_status[bulk_id].get("status", "unknown")
        return success({"bulk_status": bstatus}, f"Bulk download already {bstatus}")
    return error("Bulk download not found", 404)
...
def cancel_download(download_id):
    ...
    state.save_download_status()
    return success({"status": "cancelled", "title": state.download_status[download_id]['title']}, f"Download cancelled: {state.download_status[download_id]['title']}")
...
def clear_downloads():
    ...
    state.save_download_status()
    return success({"cleared_count": len(to_remove)}, f"Cleared {len(to_remove)} finished downloads")

@download_bp.route("/get_file/<download_id>/<filename>")
def get_file(download_id, filename):
    try:
        if not re.match(r"^[a-zA-Z0-9_.\-]+$", download_id):
            return error("Invalid download ID", 400)
