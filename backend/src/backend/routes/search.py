    ...
    try:
        ...
        for s in combined:
            ...
            if k not in seen and len(k) > 1:
                seen.add(k)
                unique.append(s)
        return success(unique[:6], f"Found {len(unique[:6])} suggestions")
    except Exception as e:
        logger.warning("Suggestions error: %s", e)
        return success([f"{query} song", f"{query} music", f"{query} latest"], "Fallback suggestions")
...
def spotify_search():
    ...
    try:
        ...
        if errors:
            return error(errors[0].get("message", "Spotify search error"), 502, {"details": errors})
        ...
        return success({
            ...
            "results": tracks,
            "source": "spotify",
        }, f"Found {returned} results on Spotify", {"source": "spotify"})
