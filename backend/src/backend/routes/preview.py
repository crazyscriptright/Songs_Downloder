...
from backend.routes.search import get_apis, get_spotify_client
...
def preview_url():
    ...
    try:
        ...
        if source == "YouTube":
                ...
                if data_yt:
                    ...
                    if videos:
                        ...
                        return success({
                            ...
                            "source": "YouTube",
                        }, "YouTube preview", {"source": "YouTube"})
            except Exception:
                logger.warning("YouTube preview extraction failed for video %s", vid)
            return success({
            ...
            }, "YouTube preview (fallback)", {"source": "YouTube"})
        ...
        if source == "JioSaavn":
            ...
            }, "JioSaavn preview", {"source": source})
        ...
        if source == "SoundCloud":
            ...
            }, "SoundCloud preview", {"source": source})
...
def audio_preview():
    ...
    if not cdn_url and not file_path:
        return error("Preview unavailable", 503)
    ...
    try:
        ...
        return error("Stream proxy failed", 502)
...
def get_jiosaavn_suggestions_by_pid(pid):
    ...
    if suggestions:
        return success({"pid": pid, "language": language, "suggestions": suggestions, "count": len(suggestions), "method": method_used}, f"Found {len(suggestions)} suggestions")
    return error("No suggestions available", 404, {"pid": pid, "language": language, "suggestions": [], "count": 0, "method": method_used})
...
def extract_playlist():
    ...
    try:
        ...
        return success({"videos": videos, "count": len(videos)}, f"Extracted {len(videos)} videos")
