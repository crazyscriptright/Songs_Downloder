...
import type { DirectUrlInfo, JioSaavnSuggestion, PreviewData, SearchType, Song, SoundCloudTrack, SourceId, SourceInfo } from '@/types';
...
type ToastFn = (type: 'success' | 'error' | 'info', title: string, msg: string, dur?: number) => void;

/* ------------------------------------------------------------------ */
/* SVG icon constants for the preview pane                             */
/* ------------------------------------------------------------------ */
const LINK_ICON = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline;margin-right:8px;"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg>';
...
export class ResultsContainer {
  ...
  /* ================================================================ */
  /* Display search-style results (keyword search)                     */
  /* ================================================================ */

  displayResults(data: Record<string, Song[]>): void {
    this.el.innerHTML = '';

    // Direct-URL path handled separately by displayDirectUrl
    ...
    if (sources.length === 0) {
      this.el.innerHTML = '<div class="empty-state"><i>\u2205</i><h3>No results found</h3><p>Try a different search query</p></div>';
      return;
    }
    ...
    sources.forEach((source, index) => {
      ...
      const section = createSourceSection(source.name, songs, source.id, this.searchType, this.onDownload, this.onAdvanced);
  ...
  /* ================================================================ */
  /* Display direct-URL results (URL preview + download options)       */
  /* ================================================================ */
  ...
  async displayDirectUrl(info: DirectUrlInfo): Promise<void> {
    ...
    // Build processed info
    let processedInfo = { ...info };
    let youtubeData: ReturnType<typeof convertYouTubeMusicUrl> | null = null;

    if (info.source === 'YouTube' && (info.url.includes('music.youtube.com') || info.url.includes('youtube.com') || info.url.includes('youtu.be'))) {
    ...
    if (this.searchType === 'all') {
      ...
      else if (info.url.includes('youtube.com/watch') || info.url.includes('youtu.be/')) urlMode = 'video';
    ...
    // Async load JioSaavn suggestions
    if (previewData?.source === 'JioSaavn' && previewData.pid) {
      this.loadJioSaavnSuggestions(previewData.pid, section, previewData.language || '');
    }

    // SoundCloud recommendations from preview data
    if (previewData?.source === 'SoundCloud' && previewData.soundcloud_data?.recommended_tracks?.length) {
  ...
  private buildPreviewHTML(
    ...
    // SoundCloud main track
    if (preview?.soundcloud_data && preview.source === 'SoundCloud') {
      const t = preview.soundcloud_data.main_track;
      const thumb = t.thumbnail || 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect fill="%23333"/><text x="50%" y="50%" text-anchor="middle" dy=".3em" fill="%23666">\u266A</text></svg>';
      return `
    ...
    // Generic preview with thumbnail
  ...
  private buildRecommendationsPlaceholder(preview: PreviewData | null, info: DirectUrlInfo): string {
  ...
  private async loadJioSaavnSuggestions(pid: string, section: HTMLElement, language: string): Promise<void> {
    try {
  ...
  private buildRecommendationCard(track: { title: string; artist: string; url: string; thumbnail?: string; duration?: string; plays?: number }): string {
